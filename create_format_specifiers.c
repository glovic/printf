 ...
 * specifiers intended for use by the custom_printf() function. If an invalid
 * specifier is encountered, the function returns NULL. Keep in mind that the
 * memory for this array was allocated using malloc() and should be freed using
 * safe_free() when it is no longer required to prevent memory leaks.
 *
